...
float get_cost(string item)
{
    // find the items in the list and add them up
    for (int i = 0; i < NUM_ITEMS; i++) // loops through all the menus
    {
        if (strcasecmp(item, menu[i].item) == 0) // strcasecmp == case insensitive comparison function
        {
            return menu[i].price;
        }
    }
